...
from transformers.generation.utils import GenerateEncoderDecoderOutput, GenerateBeamDecoderOnlyOutput, GenerateBeamEncoderDecoderOutput,  ModelOutput
...
class GenerateDecoderOnlyOutput(ModelOutput):
    ...
    Args:
        ...
        loss (float): The loss of the generation.
    """
...
class CustomGeneration(GenerationMixin):
    ...
    def can_generate():
         """
        Indicates that this class supports text generation functionality.

        Returns:
            bool: Always returns True, indicating generation capability.
        """
         return True
        ...
        **model_kwargs,
        ...
        while self._has_unfinished_sequences(
            ...
            if model_kwargs['labels'] is not None:
                model_inputs['labels'] = self.prep_labels(model_kwargs['labels'],gen_len)
            ...
            if gen_len >= model_kwargs['labels'].size(1):
                break
...
class LlamaForCausalLMCustom(LlamaPreTrainedModel, CustomGeneration):
    ...
    def forward(
        ...
        **loss_kwargs,
        ...
        if labels is not None:
            loss = self.loss_function_for_gen(logits, labels, self.config.vocab_size,num_items_in_batch=logits.size(0), **loss_kwargs)
