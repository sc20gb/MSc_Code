...
def feature_aliginment_training_step_1_CPU_GPU_SPLIT(clip_parameters,optim_parameters,connector_llm_parameters,per_warm,image_size,batch_size,rand_seed,MAX_EPOC,MAX_LENGTH,VERSION,save=False):
    # CHECK GPU SUPPORT AND ASSIGN DEVICE
    if torch.cuda.is_available():
        # Get the number of GPUs available
        gpu_count = torch.cuda.device_count()
        ...
        device_llm = torch.device("cuda:0")  # Use GPU for LLM
        device_vit = torch.device("cpu")  # Use CPU for ViT encoder
    else:
        print("CUDA is not available. Training will proceed on CPU.")
    ...
    # LOAD DATA
    test_loader, train_loader, validate_loader = load_data(
        transforms.Compose([
            transforms.Resize((image_size, image_size)),
            transforms.ToTensor()
        ]), batch_size, rand_seed, os.path.join(os.getcwd(), 'Slake1.0')
    ...
    # Load connector and vicuna model
    connector_llm = Connector_LLM(**connector_llm_parameters, device=device_llm, MAX_LENGTH=MAX_LENGTH)

    # FREEZE vicuna TRAINING (should save memory and computation as well)
    connector_llm.vicuna.eval()

    # LOAD ViT encoder from the CLIP model
    img_encoder, logit_scale = load_ViT_img_encoder(**clip_parameters, device=device_vit, tokenizer=connector_llm.tokenizer, MAX_LENGTH=MAX_LENGTH)
    ...
    for n in range(1, MAX_EPOC + 1):
        ...
        for image_tensor, mask_tensor, question, answer in train_loader:
            ...
            # Get image features from the img encoder (on CPU)
            image_features = img_encoder(image_tensor.to(device_vit))

            # Move image features to GPU for LLM processing
...
def feature_aliginment_training_step_1_GPU_SPLIT(clip_parameters,optim_parameters,connector_llm_parameters,per_warm,image_size,batch_size,rand_seed,MAX_EPOC,MAX_LENGTH,VERSION,save=False):
...
feature_aliginment_training_step_1_GPU_SPLIT(clip_parameters,optim_parameters,connector_llm_parameters,per_warm=0.2,image_size=224,batch_size=2,rand_seed=42,MAX_EPOC=3,MAX_LENGTH=256,VERSION=2000, save=True)