...
Ensures that the MLP doesn't distort features so far that they cannot be recovered. Regularises the LLM'S effect on selecting relevant information.
...
- Structural Preservation: Forces projected embeddings to maintain consistent semantic relationships. When image features are projected, related concepts remain close in the embedding space, while unrelated concepts remain distant. This preserves the inherent visual relationships (e.g., "cat" and "dog" remain closer to each other than to "aeroplane") throughout the projection process. The LLM can better understand visual semantics when processing multimodal inputs by maintaining these relationships.
